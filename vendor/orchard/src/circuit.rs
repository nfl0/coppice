...
struct AddressPoints {
    ...
    nf_old: halo2_gadgets::ecc::X<pallas::Affine, EccChip<OrchardFixedBases>>,
    v_old: halo2_proofs::circuit::AssignedCell<NoteValue, pallas::Base>,
    ak_p: NonIdentityPoint<pallas::Affine, EccChip<OrchardFixedBases>>,
}
...
impl plonk::Circuit<pallas::Base> for Circuit {
    ...
    fn synthesize(
        ...
        let addrs = self.synthesize_base(&config, &mut layouter, true)?;
...
/// Reusable non-consensus circuit that proves ownership and membership of an Orchard note while
/// keeping the note commitment and nullifier private. This composes the unchanged Action
/// constraints; it is not used by transaction validation.
#[allow(missing_docs)]
pub mod bond {
    use super::*;
    ...
    pub const PROTOCOL_INPUT: usize = 2;
    pub const CONTEXT_INPUT: usize = 3;
    pub const OWNER_BINDING_INPUT: usize = 4;
    pub const BOND_TAG_INPUT: usize = 5;
    ...
    impl BondCircuit {
        ...
        pub fn from_action_context(
            ...
            let expected_ak: crate::keys::SpendValidatingKey = (&ask).into();
            let note_ak: crate::keys::SpendValidatingKey = spend.fvk.clone().into();
            if expected_ak != note_ak {
                return None;
            }
            Circuit::from_action_context(spend, output_note, alpha, rcv, circuit_version).map(
                |action| Self {
                    action,
    ...
    impl plonk::Circuit<pallas::Base> for BondCircuit {
        type Config = BondConfig;
        type FloorPlanner = floor_planner::V1;

        fn without_witnesses(&self) -> Self {
            Self {
                action: Circuit::empty(self.action.circuit_version),
                ask: Value::unknown(),
        ...
        fn configure(meta: &mut plonk::ConstraintSystem<pallas::Base>) -> Self::Config {
            let action = Config::configure(meta);
            let q_minimum = meta.selector();
            meta.create_gate("Coppice bond minimum value", |meta| {
                let q = meta.query_selector(q_minimum);
                let value = meta.query_advice(action.advices[0], Rotation::cur());
                let minimum = meta.query_advice(action.advices[1], Rotation::cur());
                let difference = meta.query_advice(action.advices[2], Rotation::cur());
                Constraints::with_selector(
                    q,
                    [("value = minimum + difference", value - minimum - difference)],
                )
            });
            BondConfig { action, q_minimum }
        }
        ...
        fn synthesize(
            ...
            let cells = self
                .action
                .synthesize_base(&config.action, &mut layouter, false)?;

            // Prove possession of ask by deriving the exact ak already constrained into the note.
            let ecc_chip = config
                .action
                .ecc_chip(self.action.circuit_version.halo2_version());
            let ask = ScalarFixed::new(
                ecc_chip.clone(),
                layouter.namespace(|| "bond ask"),
                self.ask,
            )?;
            let spend_auth_g =
            ...
            derived_ak.constrain_equal(layouter.namespace(|| "ask owns note"), &cells.ak_p)?;

            // Constrain value - B to a canonical 64-bit nonnegative integer.
            let difference = self
                .action
                .v_old
                .map(|v| pallas::Base::from(v.inner().saturating_sub(self.minimum_value)));
            let running = config.action.range_check.witness_check(
                layouter.namespace(|| "bond value difference"),
                difference,
                7,
                true,
            )?;
            config.action.range_check.copy_short_check(
                layouter.namespace(|| "bond value top four bits"),
                running[6].clone(),
                4,
            )?;
            layouter.assign_region(
                || "bond minimum relation",
                |mut region| {
                    cells.v_old.copy_advice(
                        || "note value",
                        &mut region,
                        config.action.advices[0],
                        0,
                    )?;
                    region.assign_advice_from_instance(
                        || "minimum",
                        ...
                        MINIMUM_VALUE_INPUT,
                        config.action.advices[1],
                        0,
                    )?;
                    running[0].copy_advice(
                        || "difference",
                        &mut region,
                        config.action.advices[2],
                        0,
                    )?;
                    config.q_minimum.enable(&mut region, 0)
                },
            )?;
            ...
            let bond_tag = PoseidonHash::<
                ...
                2,
            >::init(
                config.action.poseidon_chip(),
                layouter.namespace(|| "bond tag init"),
            )?
            .hash(
                layouter.namespace(|| "bond tag"),
                [bond_domain, cells.nf_old.inner().clone()],
            )?;
            layouter.constrain_instance(bond_tag.cell(), config.action.primary, BOND_TAG_INPUT)?;

            let owner_binding = assign_free_advice(
                layouter.namespace(|| "external owner binding"),
                config.action.advices[0],
                self.owner_binding,
            )?;
            ...
            let protocol = assign_free_advice(
                layouter.namespace(|| "protocol binding"),
                config.action.advices[0],
                self.protocol_binding,
            )?;
            layouter.constrain_instance(protocol.cell(), config.action.primary, PROTOCOL_INPUT)?;
            let context = assign_free_advice(
                layouter.namespace(|| "context binding"),
                config.action.advices[0],
                self.context_binding,
            )?;
            layouter.constrain_instance(context.cell(), config.action.primary, CONTEXT_INPUT)?;
            Ok(())
        }
    }
}
